...
*/
					...
					if ($cas == 1 && $cptLineGroupe == 3 && (preg_match("@^    ([a-zA-Z0-9]*) @", $buffer) OR preg_match("@^    \"\"@", $buffer))) // on cherche la 3 eme ligne. le old ou le # t SAUF NEW !
					{
...
*/
					...
					if ($cas == 2 && $cptLineGroupe == 1 && preg_match("@^    # ([a-zA-Z0-9]{1,})/@", $buffer)) // Si c'est la ligne de début du groupe d'une traduction :
					{
...
*/
					...
					if ($targetText)
					{
						...
						if ($test1 == 'new ""' OR preg_match('@^([a-zA-Z0-9]{1,}) ""@', $test1) OR preg_match('@^""$@', $test1)) // Vide à traduire.
							$etat = 0;
