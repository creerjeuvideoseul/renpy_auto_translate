		...
		if (substr($fileTranslate, -3) == "rpy")
		{
			...
			while ($row = $result->fetch_assoc()) {
				...
				if ($row['tt_translate'] && $row['tt_etat'] == 2)
				{
					...
					elseif ($texteSource && preg_match("@^# ([a-zA-Z0-9]{1,}) \"@", trim($texteSource)))
						$texteSourceClean = preg_replace("@^# ([a-zA-Z0-9]{1,}) \"@", "", trim($texteSource)); // On retire le cas du t
 					else
					...
					elseif ($texteTraduit && preg_match("@^([a-zA-Z0-9]{1,}) \"@", trim($texteTraduit)))
						$texteTraduitClean = preg_replace("@^([a-zA-Z0-9]{1,}) \"@", "", trim($texteTraduit)); // On retire le cas du t
 					else
